...
def get_corr(history, target_history):
    ...
    if total_years >= 15 and target_total_years >= 15:
        ...
        corr_data['end_date'] = end_date
        corr_data['corr_yearly'] = history.loc[history['date'] > fifteen_year_ago].set_index('date')['price'].resample('Y').last().pct_change().corr(target_history.set_index('date')['price'].resample('Y').last().pct_change())
        corr_data['corr_monthly'] = history.loc[history['date'] > fifteen_year_ago].set_index('date')['price'].resample('M').last().pct_change().corr(target_history.set_index('date')['price'].resample('M').last().pct_change())
        corr_data['corr_weekly'] = history.loc[history['date'] > fifteen_year_ago].set_index('date')['price'].resample('W').last().pct_change().corr(target_history.set_index('date')['price'].resample('W').last().pct_change())
        corr_data['corr_daily'] = history.loc[history['date'] > fifteen_year_ago].set_index('date')['price'].resample('D').last().pct_change().corr(target_history.set_index('date')['price'].resample('D').last().pct_change())
        corr_data = pd.DataFrame([corr_data])
        corr_data_list.append(corr_data)
    ...
    if total_years >= 10 and target_total_years >= 10:
        ...
        corr_data['end_date'] = end_date
        corr_data['corr_yearly'] = history.loc[history['date'] > ten_year_ago].set_index('date')['price'].resample('Y').last().pct_change().corr(target_history.set_index('date')['price'].resample('Y').last().pct_change())
        corr_data['corr_monthly'] = history.loc[history['date'] > ten_year_ago].set_index('date')['price'].resample('m').last().pct_change().corr(target_history.set_index('date')['price'].resample('M').last().pct_change())
        corr_data['corr_weekly'] = history.loc[history['date'] > ten_year_ago].set_index('date')['price'].resample('W').last().pct_change().corr(target_history.set_index('date')['price'].resample('W').last().pct_change())
        corr_data['corr_daily'] = history.loc[history['date'] > ten_year_ago].set_index('date')['price'].resample('D').last().pct_change().corr(target_history.set_index('date')['price'].resample('D').last().pct_change())
        corr_data = pd.DataFrame([corr_data])
        corr_data_list.append(corr_data)
    ...
    if total_years >= 5 and target_total_years >= 5:
        ...
        corr_data['end_date'] = end_date
        corr_data['corr_yearly'] = history.loc[history['date'] > five_year_ago].set_index('date')['price'].resample('Y').last().pct_change().corr(target_history.set_index('date')['price'].resample('Y').last().pct_change())
        corr_data['corr_monthly'] = history.loc[history['date'] > five_year_ago].set_index('date')['price'].resample('m').last().pct_change().corr(target_history.set_index('date')['price'].resample('M').last().pct_change())
        corr_data['corr_weekly'] = history.loc[history['date'] > five_year_ago].set_index('date')['price'].resample('W').last().pct_change().corr(target_history.set_index('date')['price'].resample('W').last().pct_change())
        corr_data['corr_daily'] = history.loc[history['date'] > five_year_ago].set_index('date')['price'].resample('D').last().pct_change().corr(target_history.set_index('date')['price'].resample('D').last().pct_change())
        corr_data = pd.DataFrame([corr_data])
        corr_data_list.append(corr_data)
...
if __name__ == '__main__':
    symbols = [x.split('_')[0] for x in os.listdir("./downloads/history/etf") if x.endswith('csv')][:2]
